...
chunkWorker.postMessage(['Hello World'])
...
for (let x = 0; x < WORLD_SIZE; x++) {
    for (let z = 0; z < WORLD_SIZE; z++) {
        ...
        var data = {
            x: x - WORLD_SIZE / 2 + 0.5,
            z: z - WORLD_SIZE / 2 + 0.5,
            chunkDepth: 10,
            chunkHeight: 10,
            chunkSize: 0.01,
            seed: seed
        ...
        xhr.addEventListener('load', buildChunk);
    }
}

function buildChunk(e) {
    let data = JSON.parse(e.target.responseText)
    console.log(data)
    let chunkClone = new Chunk(data.globalX, data.globalZ, data.chunkDepth, simplex, data.genHeight, data.genSize, data.materials, data.materialsOrder, data.geometry, data.blocks)
    console.log(chunkClone)
}
